//! Functional veth throughput measurement for the live AF_PACKET data path.
//!
//! This test is intentionally a privileged, opt-in measurement rather than a
//! normal unit test.  It creates no interface itself: the companion shell
//! harness creates a fresh veth pair and supplies both interface names.
//! Independent raw AF_PACKET sockets on the peer continuously send one
//! pre-built frame each, while `AfPacketIo::receive` owns and recycles every
//! packet on the target.
...
mod linux {
    ...
    const SOURCE_MAC: [u8; 6] = [0x02, 0x72, 0x75, 0x73, 0x74, 0x51];
    ...
    // These layouts match the Linux userspace ABI used by the `sendmmsg`
    // symbol on the measured x86_64 host. The corresponding installed-header
    // definitions are cited in the measurement report; no external crate is
    // needed here.
    #[repr(C)]
    struct Iovec {
        iov_base: *mut c_void,
        ...
        fn from_environment() -> Self {
            let backend =
                env::var("RUSTER_THROUGHPUT_BACKEND").unwrap_or_else(|error| match error {
                    env::VarError::NotPresent => "af_packet".to_owned(),
            ...
            if backend != "af_packet" {
                panic!(
                    "AF_PACKET throughput test requires backend=af_packet; requested {backend:?}; use the harness for AF_XDP or backend selection"
                );
            }
    ...
    impl Drop for SenderGuard {
        fn drop(&mut self) {
            ...
            for handle in self.handles.drain(..) {
                if handle.join().is_err() {
                    eprintln!("AF_PACKET throughput sender thread panicked during cleanup");
                }
            }
        }
    }
    ...
    fn require_privileged_execution() {
        ...
        // narrower capability set.  This makes the measurement contract
        // unambiguous and matches the privileged E2E policy.
    ...
    fn run_measurement(settings: &Settings) -> Measurement {
        if let Err(error) = AfPacketPlatform::ensure_supported() {
            panic!("AF_PACKET platform is unsupported: {error}");
        }
        ...
        .unwrap_or_else(|error| panic!("AF_PACKET ring configuration failed: {error:?}"));
        ...
        .unwrap_or_else(|error| panic!("AF_PACKET sender thread setup failed: {error}"));
        ...
        while Instant::now() < warmup_deadline {
            ...
            let _ = io
                ...
                .unwrap_or_else(|error| panic!("AF_PACKET warmup wait failed: {error:?}"));
            let _ = receive_and_recycle(&mut io, settings.frame_size, None);
        }
        ...
        if counters.phase.load(Ordering::Acquire) == PHASE_FAILED {
            ...
            if let Err(error) = sender.stop() {
                panic!("AF_PACKET sender cleanup failed: {error}");
            }
            panic!("AF_PACKET sender failed during warmup with errno {fatal_errno}");
        }
        ...
        while Instant::now() < requested_end {
            ...
            let _ = io
                .wait_for_rx(remaining.min(POLL_SLICE))
                .unwrap_or_else(|error| panic!("AF_PACKET measurement wait failed: {error:?}"));
        ...
        let fatal_errno = counters.fatal_errno.load(Ordering::Acquire);
        if fatal_errno != 0 {
            panic!("AF_PACKET sender failed with errno {fatal_errno}");
        }
        record_interval(
            &mut ranges,
    ...
    fn drain_until_quiet(io: &mut AfPacketIo, frame_size: usize) {
        ...
        while Instant::now() < deadline && empty_batches < 3 {
            let _ = io
                ...
                .unwrap_or_else(|error| panic!("AF_PACKET quiet-drain wait failed: {error:?}"));
    ...
    fn receive_and_recycle(
        ...
        // A TPACKET_V3 block can contain RX_FRAME_COUNT packets over the whole
        // ring. usize::MAX is intentional: receive() clamps to the acquired
        // block, and this value guarantees that every descriptor in that
        // block is exposed to this loop before finish().
        let mut batch = io
            .receive(usize::MAX)
            .unwrap_or_else(|error| panic!("AF_PACKET PacketIo::receive failed: {error:?}"));
        ...
        assert_eq!(
            ...
            "every acquired AF_PACKET descriptor must be leased and recycled before finish"
        );
        totals
    }
    ...
    mod tests {
        ...
        fn build_record_is_machine_parseable() {
            ...
            assert_machine_readable_record_line(&record);
            assert!(record.contains("backend=af_packet"));
        }
    }
...
#[test]
#[ignore = "requires Linux and an explicit root veth throughput run"]
fn veth_throughput_af_packet() {
    panic!("AF_PACKET veth throughput is unsupported on this operating system");
}
