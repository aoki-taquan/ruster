...
| ICMP4-012A1 selected-prefix boundary suppression | RFC 1812 §4.3.2.7, §§4.2.3.1, 5.3.5 | `selected_gateway_prefix_suppresses_remote_boundaries_but_lpm_host_routes_win` | implemented | gateway-routed `/24`のnetwork/directed broadcastも抑止。LPM-selected `/32`を優先し、`/31` endpointを許可 |
