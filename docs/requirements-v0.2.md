...
| NAT44-019B read-only ICMP NAT session | RFC 5508 REQ-6, architecture contract | `backend_reject_and_clock_regression_leave_icmp_lookup_state_unchanged` | implemented | lookup成功/drop/backend rejectでmapping/peer/session/last time/counter/watermarkを変更しない |
...
| NAT44-019D opaque trailing bytes boundary | RFC 5508 REQ-3(d), local partial profile | `tcp_total_length_prevents_opaque_trailing_bytes_from_becoming_a_checksum` | implemented | 引用Total Length外をtransport checksumと誤認しないがRFC 4884 object検出・解析は非主張 |
| NAT44-019E external source admission and preservation | RFC 5508 REQ-4, RFC 1812 §5.3.7 | `outer_source_admission_rejects_non_hosts_local_and_inside_routes_atomically` | implemented | host-unicastかつreverse outsideだけ許可し中継sourceは引用remoteと不一致でもouter sourceを保存 |
